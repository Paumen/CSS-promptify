...
Version: 1.1 (merged + tuned to your preferences)  
Owner: (you)  
...
### 9.1 Editor & input
- **FR-IN-01:** Syntax-highlighted CSS editor with line numbers.
- **FR-IN-02:** Paste input and file upload (`.css`).
- **FR-IN-03:** “Analyze” runs the rule engine and returns issues + stats.
- **FR-IN-04:** No automatic formatting on paste; only on selected fix application.
...
- **FR-FIX-02:** User can apply fixes:
  ...
  - per group
  - per severity
- **FR-FIX-03:** Diff preview before apply.
- **FR-FIX-04:** Tool maintains an **Applied Fixes list** in the session:
  - each applied fix can be toggled off to revert
  - revert returns the code to the state as if that fix never happened
...
- **FR-COMMENT-02:** Inline comments are inserted only for **applied** fixes and only when toggle is ON.
...
### 13.2 Applied fix tracking (session)
Maintain an internal list of applied patches with IDs so they can be reverted cleanly:
- fix_id
- rule_id
- original range/text
- replacement text
- created inline comment text (if enabled)
...
### 14.3 Fixing + revert
- **AC-06:** User can apply one fix, then revert it, and code returns to the pre-fix state.
- **AC-07:** Reverting one fix does not revert other fixes.
- **AC-08:** After apply/revert cycles, output remains stable (no formatting drift).

### 14.4 Inline comments
- **AC-09:** When comments toggle is ON, applied fixes add brief inline comments with old value/property reference.
...
### 14.5 Stats
- **AC-12:** UI shows tokens/lines/chars before and after. Values update after applying/reverting fixes.

### 14.6 Mobile
- **AC-13:** On mobile, user can paste, analyze, filter, apply, revert, and copy without UI blocking actions.
